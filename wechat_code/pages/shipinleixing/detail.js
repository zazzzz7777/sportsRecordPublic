...
const {
data
} = await detail("shipinleixing",id)
this.setData({
payAuth:utils.isAuthFront('shipinleixing','支付')
})
...
const resUpdate = await update('shipinleixing', predetailList)
if (resUpdate.code == 0) {
this.setData({
predetailList,
...
wx.setStorageSync('paytable','shipinleixing');
...
async reply() {
const detailList = this.data.detailList
const res = await update("shipinleixing", detailList)
if (res.code == 0) {
setTimeout(function () {
wx.showToast({
title: '回复成功',
