...
this.setData({
addAuth: utils.isAuth("shipinleixing", "新增") || utils.isAuthFront("shipinleixing", "新增"),
delAuth: utils.isAuth("shipinleixing", "删除") || utils.isAuthFront("shipinleixing", "删除"),
editAuth: utils.isAuth("shipinleixing", "修改") || utils.isAuthFront("shipinleixing", "修改"),
})
if(options?.userid) {
this.setData({
...
if(this.data.shipinleixing){
    searchForm['shipinleixing'] = '%' + this.data.shipinleixing + '%'
}


let res = {};
if(this.data.userid) {
res = await page(`shipinleixing`, searchForm);
} else {
res = await list(`shipinleixing`, searchForm);
}
...
wx.navigateTo({
url: `/pages/shipinleixing/update-and-add`
})
},
searchListHandler(e) {
this.setData({
...
complete: async (res) => {
    ...
    if (res.confirm) {
        ...
        const res = await deleteData("shipinleixing",[id])
        console.log(res);
        if (res.code == 0) {
            this.getData()
        }
...
wx.navigateTo({
url: `/pages/shipinleixing/update-and-add?id=${id}`
})
},
async detailBtn(e) {
...
wx.navigateTo({
url: `/pages/shipinleixing/detail`
})
...
obj['limit']=this.data.pageSize
    if(this.data.userid){
        resList = await page("shipinleixing",obj)
    }else{
         resList= await list("shipinleixing",obj)
    }
